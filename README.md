...
AI Floorplan to 3D 將 2D 建築平面圖影像自動解析為結構化資料，並即時生成可互動的 3D 場景。適合建築、室內設計與 PropTech 團隊用於快速視覺化提案，也可作為「平面圖到 3D」與自然語言場景操控的參考實作。

## About (EN)

AI Floorplan to 3D converts 2D architectural floorplan images into structured data and interactive 3D scenes in real time. It is useful for architecture and interior design teams that need rapid visualization and natural-language scene edits.
